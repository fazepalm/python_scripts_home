...
REF_PASS_LIST = [ 'specular', 'lighting' ]
IGNORE_LIST = [ 'rgba' ]
...
#            R_Dot = nuke.toNode( R_DotName )
        ...
        if not nuke.exists( CG_DotName_01 ):
            CG_Dot_01 = nuke.nodes.Dot( inputs=[ C_Dot ], name = CG_DotName_01, ypos = C_DotYPos + 100, xpos = C_DotXPos )
...
#            MM_Dot = nuke.toNode( MM_DotName )
        ...
        if not nuke.exists( L_DotName_01 ):
            L_Dot_01 = nuke.nodes.Dot( inputs=[ CG_Dot_01 ], name = L_DotName_01, ypos = CG_DotXPos_01, xpos = CG_DotXPos_01 - 300 )
        ...
        if CG_PASSES == 1:
            if REMAINING_PASSES != []:
                if nuke.exists( CG_Dot_02.name() ):
                    for CG_count, CG_CHANNEL in enumerate( REMAINING_PASSES ):
                        CG_SHUFFLE_NAME = compPrefix + "_" + CG_CHANNEL
                        if not nuke.exists( CG_SHUFFLE_NAME ):
                            if CG_count == 0:
                                CG_SHUFFLE = shuffleLayer( CG_Dot_01, CG_CHANNEL, CG_SHUFFLE_NAME, CG_CHANNEL )

                            else:
                                if nuke.exists( P_CG_SHUFFLE.name() ):
                                    CG_SHUFFLE = shuffleLayer( P_CG_SHUFFLE, CG_CHANNEL, CG_SHUFFLE_NAME, CG_CHANNEL )

                            CG_SHUFFLE_W = CG_SHUFFLE.screenWidth()
                            CG_SHUFFLE_H = CG_SHUFFLE.screenHeight() / 2
                            CG_SHUFFLE.setXYpos( ( CG_DotXPos_01 + CG_SHUFFLE_W ) + ( CG_SHUFFLE_X_DIST *
                                                   ( CG_count + 1 ) ), CG_DotYPos_01 - CG_SHUFFLE_H )
