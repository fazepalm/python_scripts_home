...
def getSetShuffleVariables( CG_SHUFFLE, CG_count, rootNode, rootXpos, rootYpos ):
    rootNodeClass = rootNode.Class()
    CG_SHUFFLE_W = CG_SHUFFLE.screenWidth() / 2
    CG_SHUFFLE_H = CG_SHUFFLE.screenHeight() / 2
    CG_SHUFFLE_X_VAL = ( rootXpos + CG_SHUFFLE_W ) + ( CG_SHUFFLE_X_DIST )
    if rootNodeClass == 'Dot':
        CG_SHUFFLE_Y_VAL = rootYpos - CG_SHUFFLE_H
    else:
        CG_SHUFFLE_Y_VAL = rootYpos

    CG_SHUFFLE.setXYpos( CG_SHUFFLE_X_VAL, CG_SHUFFLE_Y_VAL  )
    CG_SHUFFLE_XPos, CG_SHUFFLE_YPos = getNodePos( CG_SHUFFLE )

    return CG_SHUFFLE_W, CG_SHUFFLE_H, CG_SHUFFLE_XPos, CG_SHUFFLE_YPos

def shuffleFromList( CG_PASS_LIST, rootNode_01, rootXpos_01, rootYpos_01 ):
    shuffleDict = {}
    shuffleVarList = []
    for CG_count, CG_CHANNEL in enumerate( CG_PASS_LIST ):
        ...
        if not nuke.exists( CG_SHUFFLE_NAME ):
            ...
            CG_SHUFFLE_W, CG_SHUFFLE_H, CG_SHUFFLE_XPos, CG_SHUFFLE_YPos = getSetShuffleVariables( CG_SHUFFLE,
                                                                                                   CG_count,
                                                                                                   rootNode,
                                                                                                   rootXpos,
                                                                                                   rootYpos )
        ...
        if CG_PASSES == 1:
            if CG_CHANNEL_LIST != []:
                ...
                if nuke.exists( CG_Dot_02.name() ):
                    ...
                    CG_PASS_DICT_01 = shuffleFromList( CG_PASS_LIST_01, CG_Dot_02, CG_DotXPos_02, CG_DotYPos_02  )
                    if CG_PASS_DICT_01 != {}:
                        for CG_SHUFFLE_01_COUNT, ( CG_SHUFFLE_01, CG_SHUFFLE_VAL_LIST_01 ) in enumerate( CG_PASS_DICT_01.iteritems() ):
                            ...
                            if not nuke.exists( CG_SHUFFLE_DOT_NAME_01 ):
                                CG_SHUFFLE_DOT_01 = nuke.nodes.Dot(
                                                                name = CG_SHUFFLE_DOT_NAME_01,
                                                                ypos = CG_SHUFFLE_YPos + CG_SHUFFLE_H + UTIL_SHUFFLE_Y_DIST ,
                                                                xpos = CG_SHUFFLE_XPos )
                                CG_SHUFFLE_DOT_01_XPos, CG_SHUFFLE_DOT_01_YPos = getNodePos( CG_SHUFFLE_DOT_01 )

                                if CG_SHUFFLE_01_COUNT == 0:
                                    CG_SHUFFLE_DOT_01.setInput( 0, CG_Dot_02 )
                                else:
                            ...
                            if nuke.exists( CG_SHUFFLE_DOT_01.name() ) and nuke.exists( CG_SHUFFLE_01.name() ):
                    ...
                    CG_PASS_DICT_02 = shuffleFromList( CG_PASS_LIST_02, CG_Dot_03, CG_DotXPos_03, CG_DotYPos_03  )

        #breakout UTIL
        if UTIL == 1:
            if UTIL_CHANNEL_LIST != []:
                UTIL_PASS_DICT_01 = shuffleFromList( UTIL_CHANNEL_LIST, UTIL_Dot_01, UTIL_DotXPos_01, UTIL_DotYPos_01  )                                                       
